...
def search():
    ...
    if form.validate_on_submit():
        ...
        if patients.distinct("medical_record_number") == []:
            ...
            return redirect(
                url_for("doctor.patient", mr_num=patients[0]["medical_record_number"])
            )
...
def patient(mr_num: str) -> str:
    ...
    vitals_recorded_at = patient["created_at"].strftime("%d %B %Y %H:%M")
    ...
    return render_template(
        ...
        vitals_recorded_at=vitals_recorded_at,
        date=date,
        time=time,
    )
...
@doctor.route(
    "/patient/<string:mr_num>/record/medical-history", methods=["GET", "POST"]
)
@login_required
def record_medical_history(mr_num: str) -> str:
    ...
    if request.method == "POST":
        ...
        if allergies == "yes":
            ...
            allergies = {"drug": None, "pollen": None, "dust": None, "other": None}
        ...
        if lmp_date:
            lmp_date = (
                datetime.datetime.strptime(lmp_date, "%Y-%m-%d")
                .date()
                .strftime("%d %B %Y")
            )
            lmp_date_temp = datetime.datetime.strptime(lmp_date, "%d %B %Y").date()

            gestation_age = (datetime.datetime.now().date() - lmp_date_temp).days // 7
        ...
        collection.update_one(
            {"medical_record_number": mr_num}, {"$set": medical_history_form_data}
        )
        ...
        if "audio" in request.files:
            ...
            audio = Audio(
                audio_type="medical_history",
                file=audio_path,
                medical_record_number=mr_num,
            )

            t = threading.Thread(target=audio.add_audio_and_transcribe, args=())
            t.start()
...
@doctor.route("/patient/<string:mr_num>/record/family-history", methods=["GET", "POST"])
@login_required
...
@doctor.route(
    "/patient/<string:mr_num>/record/socioeconomic-history", methods=["GET", "POST"]
)
@login_required
def record_socioeconomic_history(mr_num: str) -> str:
    ...
    if request.method == "POST":
        if "audio" in request.files:
            ...
            audio = Audio(
                audio_type="socioeconomic_history",
                file=audio_path,
                medical_record_number=mr_num,
            )
    ...
    if not audio_success:
        error_response = make_response(
            render_template(
                "record_socioeconomic_history.html",
                patient=patient,
                mr_num=mr_num,
                nurse_name=nurse_name,
                date=date,
...
@doctor.route(
    "/patient/<string:mr_num>/record/previous-pregnancy", methods=["GET", "POST"]
)
@login_required
def record_previous_pregnancy(mr_num: str) -> str:
    ...
    if not audio_success:
        error_response = make_response(
            render_template(
                "record_previous_pregnancy.html",
                patient=patient,
                mr_num=mr_num,
                nurse_name=nurse_name,
                date=date,
...
@doctor.route(
    "/patient/<string:mr_num>/record/condition-at-booking", methods=["GET", "POST"]
)
@login_required
def record_condition_at_booking(mr_num: str) -> str:
    ...
    if request.method == "POST":
        if "audio" in request.files:
            ...
            audio = Audio(
                audio_type="condition_at_booking",
                file=audio_path,
                medical_record_number=mr_num,
            )
    ...
    if not audio_success:
        error_response = make_response(
            render_template(
                "record_condition_at_booking.html",
                patient=patient,
                mr_num=mr_num,
                nurse_name=nurse_name,
                date=date,
...
@doctor.route(
    "/patient/<string:mr_num>/record/present-pregnancy", methods=["GET", "POST"]
)
@login_required
def record_present_pregnancy(mr_num: str) -> str:
    ...
    if request.method == "POST":
        ...
        pres_preg = {"lmp_date": lmp_date, "gestation_age": gestation_age, "edd": edd}

        collection = DB["patients"]

        collection.update_one({"medical_record_number": mr_num}, {"$set": pres_preg})
        ...
        if "audio" in request.files:
            ...
            audio.save(audio_path)
            audio = Audio(
                audio_type="present_preg", file=audio_path, medical_record_number=mr_num
            )
...
@doctor.route("/patient/<string:mr_num>/record/proposed-plan", methods=["GET", "POST"])
@login_required
def record_proposed_plan(mr_num: str) -> str:
    ...
    if request.method == "POST":
        ...
        if next_follow_up_date:
            next_follow_up_date = (
                datetime.datetime.strptime(next_follow_up_date, "%Y-%m-%d")
                .date()
                .strftime("%d %B %Y")
            )
        ...
        collection.update_one(
            {"medical_record_number": mr_num}, {"$set": proposed_plan_form_data}
        )
...
def get_cached_transcription(audio_type: str, medical_record_number: str) -> str:
    audio = Audio(
        audio_type=audio_type, file=None, medical_record_number=medical_record_number
    )
...
def get_cached_clarification_response(file_path: str, transcription: str) -> str:
    prompting_engine = Prompting(file_path=file_path, transcription=transcription)
...
@doctor.route(
    "/patient/<string:mr_num>/transcribe/medical-history", methods=["GET", "POST"]
)
@login_required
...
@doctor.route(
    "/patient/<string:mr_num>/transcribe/family-history", methods=["GET", "POST"]
)
@login_required
...
@doctor.route(
    "/patient/<string:mr_num>/transcribe/socioeconomic-history", methods=["GET", "POST"]
)
@login_required
...
@doctor.route(
    "/patient/<string:mr_num>/transcribe/previous-pregnancy", methods=["GET", "POST"]
)
@login_required
...
@doctor.route(
    "/patient/<string:mr_num>/transcribe/condition-at-booking", methods=["GET", "POST"]
)
@login_required
...
@doctor.route(
    "/patient/<string:mr_num>/transcribe/present-pregnancy", methods=["GET", "POST"]
)
@login_required
...
@doctor.route(
    "/patient/<string:mr_num>/transcribe/proposed-plan", methods=["GET", "POST"]
)
@login_required
